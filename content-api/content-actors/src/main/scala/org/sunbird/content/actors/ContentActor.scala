...
class ContentActor @Inject() (implicit oec: OntologyEngineContext, ss: StorageService) extends BaseActor {
	...
	private def validateCurriculumRequest(request: Request): Unit ={
		...
		request.put("createdBy", request.getRequest.getOrDefault("createdBy", ""))
		request.put("createdFor", request.getRequest.getOrDefault("createdFor", ""))
		request.put("framework", request.getRequest.getOrDefault("framework", ""))
		request.put("organisation", request.getRequest.getOrDefault("organisation", ""))
	}
}
